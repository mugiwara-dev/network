...
// Hardware model components map (no GPU — integrated graphics on i3-10100)
const MODELS = {
...
function SceneContent() {
  ...
  // Find the next tutorial step (first uninstalled step)
  ...
  return <>
    ...
    <directionalLight position={[5,10,3]} intensity={3.5} castShadow shadow-mapSize={[2048,2048]}/>
    ...
    <hemisphereLight args={['#4a6a8a','#0a0f18',1.2]}/>
    <fog attach="fog" args={['#050b18',22,45]}/>

    <Workbench/>
    <Chassis powerOnState={powerOnState}/>
    <Motherboard ramSlotUnlocked={ramSlotUnlocked} onSlotClick={toggleRamSlot} installedComponents={installedIds} powerOn={powerOnState === 'success'}/>

    {/* Ghost slot outlines — shows WHERE to place each component */}
    <GhostSlots installedComponents={installedIds} selectedComponent={selectedComponent} nextStep={nextStep}/>

    {/* Hardware models — pass placed/screw props to screwable components */}
    ...
    {/* Case fans with custom props */}
    <CaseFanModel installed={get(C.FAN_FRONT)?.installed} powerOn={powerOnState === 'success'} position={INV[C.FAN_FRONT]}
      color="#38bdf8" slotKey="fan_front" {...ev(C.FAN_FRONT)}/>
    <CaseFanModel installed={get(C.FAN_REAR)?.installed} powerOn={powerOnState === 'success'} position={INV[C.FAN_REAR]}
      color="#818cf8" slotKey="fan_rear" {...ev(C.FAN_REAR)}/>

    <CableSystem installedComponents={installedIds} cableAnimations={cableAnimations} selectedComponent={selectedComponent}/>

    {/* Floating labels for all hardware */}
    {components.filter(c=>c.category!=='cable').map(comp=>(
    ...
    {/* Camera bounds: fixed orbit radius around PC case, prevents getting lost in void */}
    <OrbitControls makeDefault minPolarAngle={Math.PI/8} maxPolarAngle={Math.PI/2.1}
      minDistance={6} maxDistance={14} enablePan={false} zoomSpeed={0.8} target={[0,-0.3,0]}/>
  </>
}

export default function LabScene() {
  return <div style={{width:'100%',height:'100%',position:'absolute',inset:0}}>
    <Canvas shadows camera={{position:[8,5,8],fov:45,near:0.1,far:100}}
